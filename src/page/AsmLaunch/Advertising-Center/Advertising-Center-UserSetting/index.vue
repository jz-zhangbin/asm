...
<template>
    <div class="advuser_index">
        ...
        <div class="advuser_content">
            <h1>{{$route.query.type == 'edit' ? '编辑' : '添加'}}账户</h1>
            <div class="advuser_form">
                ...
                <div v-if="$route.query.type != 'edit'">
                    <el-select v-model="account" :disabled='!fileOk' :placeholder="selectPlaceholder" @change='selectchange(account)'>
                        <el-option v-for="(item , index) in options" :key="index" :label="item.orgName" :disabled="item.isExist" :value="index">
                        </el-option>
                    </el-select>
                ...
                <div class="advuser_istype" v-if="isLoc">
                    <h2>对公账户发票信息</h2>
                    <section>
                        <p class='advuser_form_p advuser_form_p1'>公司或产品名称</p>
                        <div>
                        ...
                        <p class='advuser_form_p advuser_form_p1'>主要联系人姓名</p>
                        <div>
                        ...
                        <p class='advuser_form_p advuser_form_p1'>结算联系人姓名</p>
                        <div>
                        ...
                        <p class='advuser_form_p advuser_form_p1'>结算联系人邮箱</p>
                        <div>
                ...
                <div class="adver_btn">
                    <el-button class="one" @click="$router.go(-1)">取消</el-button>
                    <el-button type="primary" class="two" @click="btn">{{$route.query.type == 'edit' ? '保存' : '添加'}}</el-button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    created() {
        ...
        this.routeList.push({
            name: this.$route.query.type == "edit" ? "编辑账户" : "添加账户",
    ...
    mounted() {
        ...
        if (this.$route.query.type == "edit") {
            this.AjaxGetUser(this.$route.query.id);
        }
    },
    ...
    methods: {
        ...
        AjaxGetUser(id) {
            ...
            this.$https.get(url).then(res => {
                ...
                this.fileDate = "Certificates.zip"; //文件名称
                $(".advuser_form_fil").css({
                    background: "#f7f7f7",
                    color: "#2d76ed"
                });
        ...
        AjaxEdit(obj) {
            let url =
                "/api/v1/IntellAdvertiseApi/OrgUser/UpdateOrgUserInfoModel";
            this.$https.post(url, JSON.stringify(obj)).then(res => {
                this.$router.go(-1);
            });
        }
